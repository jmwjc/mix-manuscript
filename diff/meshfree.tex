...
\end{equation}
 \DIFaddbegin \DIFadd{The consistency condition ensures that the reproducing kernel shape functions are able to reproduce the polynomial space spanned by the basis function $\boldsymbol{p}$, which is a fundamental requirement for the accuracy of the Galerkin method.
...
\DIFadd{Further, substituting }\DIFaddend Eq. \ref{rkshape} into Eq. \eqref{cc2} leads to:
...
\DIFaddbegin \DIFadd{Meshfree shape functions generally lack the Kronecker delta property, which prevents the direct imposition of essential boundary conditions.
...
\DIFaddbegin \DIFadd{The numerical integration issue caused by the loss of variational consistency between meshfree shape functions and their derivatives \mbox{%DIFAUXCMD
\cite{wu2021} }\hskip0pt%DIFAUXCMD
would not appear in the mixed formulation of Eq. \ref{ritz_Galerkin}, this is due to the fact that Eq. \ref{ritz_Galerkin} solely depends on the meshfree shape functions themselves.
...
In this subsection, 2D and 3D inf-sup tests \cite{chapelle1993}, as defined in Eq. \ref{infsup_test}, are conducted using the mixed  \DIFaddbegin \DIFadd{FE-meshfree }\DIFaddend formulations to validate the proposed inf-sup value estimator.
The 2D test considers the square domain  \DIFaddbegin \DIFadd{$\Omega = (0,1)\times (0,1)$}\DIFaddend , where the displacement is discretized by Tri3 and Quad4 with $4\times 4$, $8\times 8$, $16\times 16$ and $32\times 32$ elements, Tri6 and Quad8 with $2\times 2$, $4\times 4$, $8\times 8$ and $16\times 16$ elements, respectively. The 3D test employs a cube domain  \DIFaddbegin \DIFadd{$\Omega = (0,1)\times (0,1)\times (0,1)$ }\DIFaddend with $4\times 4$, $8\times 8$ and $16\times 16$ elements for the Tet4 and Hex8.
\DIFaddbegin \DIFadd{For pressure discretization, linear meshfree approximation with a normalized support size of $1.5$ is employed for Tri3, Quad4, Tet4 and Hex8.
For Tri6 and Quad8, a quadratic meshfree approximation with a normalized support size of $2.5$ is utilized.
}\DIFaddend In order to avoid the influence of interpolation error, uniform nodal distributions are used for pressure discretizations\DIFaddbegin \DIFadd{, for example in Figure \ref{fg:infsup_mesh}, which displays $4\times4$ Quad4 elements with $4\times3$ uniformly distributed pressure nodes}\DIFaddend .

\DIFaddbegin \begin{figure}[H]
\centering
%DIF >  \includegraphics[width=0.5\textwidth]{png/infsup_mesh.png}
\includegraphics[width=0.5\textwidth]{pdf/infsup_mesh.pdf}
\caption{\DIFaddFL{Illustration of uniform nodal distribution for inf--sup test with $n_u=5\times5$, $n_p=4\times3$}}\label{fg:infsup_mesh}
\end{figure}
...
The  \DIFaddbegin \DIFadd{optimal }\DIFaddend schemes for linear and quadratic, 2D and 3D element discretizations\DIFaddbegin \DIFadd{, namely with $r=r_{opt}$, }\DIFaddend are shown in Figure \ref{fg:mix_scheme},
 \DIFaddbegin \DIFadd{where every other displacement node is selected as the pressure node.
...
Consequently, }\DIFaddend all constraint ratios  \DIFaddbegin \DIFadd{evaluated using the discretizations in Figure \ref{fg:mix_scheme} fall within the optimal range}\DIFaddend .
The corresponding  \DIFaddbegin \DIFadd{inf--sup }\DIFaddend test results for these schemes are also marked in inf--sup test figure and show that, with mesh refinement, their $\beta$'s are always maintained at a non-negligible level.
