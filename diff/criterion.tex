...
To ensure the surjectivity of orthogonal projection and satisfactory results, the approximations of Eq.\eqref{mix_formulation} should satisfy the inf-sup condition, also known as the  \DIFaddbegin \DIFadd{Ladyzhenskaya--Babuška--Brezzi }\DIFaddend condition \cite{bathe1996}:
...
For a given $q_h \in \mathrm{Im} \mathcal{P}_h$,  \DIFaddbegin \DIFadd{since both $q_h$ and $\mathcal{P}_h \boldsymbol v_h$ belong to $\mathrm{Im} \mathcal{P}_h$, }\DIFaddend $\mathrm{Im} \mathcal{P}_h \subset Q_h$, according to the Cauchy-Schwarz inequality, we have:
\begin{equation}
...
\end{equation}
\DIFaddbegin \DIFadd{the space $V'_h \subseteq V_h \setminus \ker \mathcal{P}_h$ defined by:
}\begin{equation}
\DIFadd{V'_h = \{\boldsymbol{v}_h \in V_h \mid \mathcal{P}_h \boldsymbol{v}_h = q_h\}
}\end{equation}

\DIFaddend And the following relationship can be evidenced:
\begin{equation}\label{r12}
\sup_{\boldsymbol{v}_h \in V_h} \frac{\left| \frac{1}{3\kappa} (q_h, \mathcal{P}_h \boldsymbol{v}_h) \right|}{\|q_h\|_Q \|\boldsymbol{v}_h\|_V} = \sup_{\boldsymbol{v}_h \in V'_h} \frac{\|\mathcal{P}_h \boldsymbol{v}_h\|_Q}{\|\boldsymbol{v}_h\|_V}, \quad  \DIFaddbegin \DIFadd{\exists }\DIFaddend q_h \in \mathrm{Im} \mathcal{P}_h
\end{equation}
...
\begin{thm}
Suppose that  \DIFaddbegin \DIFadd{$P_{n_u}$ }\DIFaddend is a complete polynomial space with $n_u$ dimensions, and $V_{n_u}$ is the polynomial displacement space, $V_{n_u} = P_{n_u}^{n_d}$. The inf-sup value $\beta$ can further be bounded by:
...
\end{equation}
where $\mathcal{I}_h$ is the interpolation operator of the  \DIFaddbegin \DIFadd{displacement }\DIFaddend approximation, and correspondingly, $O(h)$ is the remainder related to $h$.
\end{thm}
...
As we can see in Eqs. \eqref{estimator} and \eqref{beta_s}, $\beta_s \ge 0$, the condition \DIFaddbegin \DIFadd{that }\DIFaddend $\beta_s$ being equal to $0$ or not determines whether the formulation can satisfy the inf-sup condition. If $\beta_s > 0$, as the mesh refines, the second term on the right-hand side of Eq. \eqref{estimator} will sharply reduce and can be ignored. In contrast, if $\beta_s = 0$, the second term will dominate, and the evaluation of $\beta$ will be dependent to $h$.
...
\begin{equation}
n\DIFaddbegin \DIFadd{_d \times n}\DIFaddend _u = \dim V_{n_u}
\end{equation}
...
\end{equation}
\DIFaddbegin \DIFadd{where $\lfloor \bullet \rfloor$ denotes the floor function. 
}\DIFaddend 
